...
def test_impulsive_always_buys():
    ...
    assert impulsiveStrategy(p, prop) is True
...
def test_demanding_requires_rent_gt_50():
    ...
    assert demandingStrategy(p, prop1) is True
    assert demandingStrategy(p, prop2) is False
...
def test_cautious_requires_reserve():
    ...
    assert cautiousStrategy(p, prop) is True
    prop2 = Property(position=1, price=150, rent=10)
    assert cautiousStrategy(p, prop2) is False
